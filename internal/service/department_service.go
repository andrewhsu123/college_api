...
func (s *DepartmentService) GetStaffVisibleDepartmentIDs(customerID, personID int) ([]int, error) {
	// 1. 获取直接分配给人员的部门权限（新逻辑：从 persons_has_department 表查询）
	directDeptIDs, err := s.repo.GetDirectDepartmentIDs(customerID, personID)
	if err != nil {
	...
	// 2. 扩展为包含所有子部门的ID列表
	visibleDeptIDs, err := s.repo.ExpandDepartmentIDs(customerID, directDeptIDs)
	if err != nil {
...
func (s *DepartmentService) buildTreeStructure(school *model.Department, depts []model.Department) []model.DepartmentNode {
	...
	// 并将其 parent_id 临时改为学校的 ID
	level3Count := 0
	for _, dept := range depts {
		if dept.TreeLevel == 3 {
			childNode := buildNode(dept)
			// 将 tree_level=3 的节点的 parent_id 临时改为学校的 ID
			childNode.ParentID = school.ID
			schoolNode.Items = append(schoolNode.Items, childNode)
			level3Count++
			println("[DEBUG] buildTreeStructure: 将机构", dept.ID, "挂在学校节点下，parent_id改为", school.ID, "，子节点数:", len(childNode.Items))
		}
	}
