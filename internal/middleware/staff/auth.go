...
func (m *AuthMiddleware) validateToken(plainToken string) (int64, error) {
	...
	if !isValidType {
		...
		return 0, fmt.Errorf("Token类型错误: 此Token属于%s，不能访问政工端接口", tokenableType)
	}
	...
	fmt.Printf("[STAFF DEBUG] Person信息 - Type: %d, Status: %d\n", personType, status)

	// 验证是否为政工类型 (person_type = 2)
	if personType != 2 {
		fmt.Printf("[STAFF DEBUG] Person类型不匹配，期望: 2(政工), 实际: %d\n", personType)
		return 0, fmt.Errorf("无权限访问: 此账号不是政工类型")
	}
	...
	fmt.Printf("[STAFF DEBUG] Token验证成功，PersonID: %d\n", personID)
	return personID, nil
}
